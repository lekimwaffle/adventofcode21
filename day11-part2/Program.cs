...
int? flashedAtStep = null;
for (int step = 0; !flashedAtStep.HasValue; step++)
{
    ...
    if (energizedPositions.Count == points.Length)
        flashedAtStep = step + 1;

    energizedPositions.ForEach(x => points[x.X, x.Y] = 0);
}

Debug.WriteLine($"The answer is {flashedAtStep.Value}");
